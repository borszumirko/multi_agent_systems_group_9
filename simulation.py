...
class Simulation:
        ...
        for i in range(n_agents):
            ...
            if not in_exit_area:
                ...
                for obstacle in obstacles:
                    if agents[i].is_in_obstacle(obstacle, radius):
            ...
            for j in range(i + 1, n_agents):
                ...
                if distance < 2 * radius:
                    ...
                    for agent_position in [positions[i], positions[j]]:
                        ...
                        if not in_exit_area:
                            agent_position[0] = max(box_left + radius,
                                                    min(agent_position[0], box_left + box_width - radius))
                            agent_position[1] = max(box_top + radius,
                                                    min(agent_position[1], box_top + box_height - radius))
    ...
    def main_loop(self):
        ...
        for i in range(AGENT_COUNT // 15):
            ...
            for j in range(15):
                ...
                starting_positions.append(
                    (BOX_LEFT + 75 - AGENT_RADIUS + col * (AGENT_RADIUS + OBSTACLE_WIDTH * 2 - AGENT_RADIUS)
                     , BOX_TOP + CORR_WIDTH + AGENT_RADIUS + row * (AGENT_RADIUS * 2 + space_between_starting_agents)))
        agents = [Agent(x, y, id) for (id, (x, y)) in enumerate(starting_positions)]

        # Obstacles
        obstacles = []
        for i in range(15):
            obstacles.append(Obstacle(BOX_LEFT + 75 + (i) * OBSTACLE_WIDTH * 2, BOX_TOP + CORR_WIDTH, OBSTACLE_WIDTH,
                                      OBSTACLE_HEIGHT))
        # Front Desk
        obstacles.append(
            Obstacle(BOX_LEFT + ((1250 + 450 + 90) // SCALING), BOX_TOP + CORR_WIDTH + (55 // SCALING), BIG_OBSTACLE_W,
                     BIG_OBSTACLE_H))
        ...
        while running:
            ...
            resolved_positions = self.resolve_positions(positions, AGENT_RADIUS, BOX_WIDTH, BOX_HEIGHT, BOX_LEFT,
                                                        BOX_TOP, obstacles, agents)
